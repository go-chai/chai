...
// ParseComment parses comment for given comment string and returns error if error occurs.
func (operation *Operation) ParseCommentChai(comment string, astFile *ast.File) error {
	commentLine := strings.TrimSpace(strings.TrimLeft(comment, "/"))
	if len(commentLine) == 0 {
		return nil
	}
	...
	case produceAttr:
		err = operation.ParseProduceComment(lineRemainder)
	case "@param":
		err = operation.ParseParamCommentChai(lineRemainder, astFile)
	case "@success", "@failure", "@response":
		err = operation.ParseResponseCommentChai(lineRemainder, astFile)
	case "@header":
		err = operation.ParseResponseHeaderComment(lineRemainder, astFile)
	case "@router":
		err = operation.ParseRouterComment(lineRemainder)
...
func (operation *Operation) ParseParamCommentChai(commentLine string, astFile *ast.File) error {
...
func (operation *Operation) ParseResponseCommentChai(commentLine string, astFile *ast.File) error {
